 ...
 *
 */
public enum SimpleConnectionPool {
	INSTANCE;
	...
	private static final String DEFAULT_DRIVER = props.getProperty(DbConfigKey.DRIVER.getVal());
	private static final String DEFAULT_USER = props.getProperty(DbConfigKey.DB_USER.getVal());
	private static final String DEFAULT_MAX_CONN = props.getProperty(DbConfigKey.MAX_CONNECTIONS.getVal());
	...
	/* throws DatabaseException */ {
		...
		this.dbpass = dbpass;
		this.drivername = (drivername == null || drivername.isEmpty()) ? DEFAULT_DRIVER : drivername;
	...
	synchronized Connection getConnection(
			String URI) /* throws DatabaseException */ {
		...
		if (freeConnections != null && !freeConnections.isEmpty()) {
			...
			try {
				...
				// LOG.info("Exception occurred in checking whether connection
				// is closed");
				// Try again recursively
				con = getConnection(URI);
			}
	...
	/**
	 *  This method returns connection pool
	 * @param con
	 * @param URI
	 */
	...
	/**
	 *  this method releases connection
